% Maglev Model 3
% 2-DOF Train on Flexible Guideway
% Finite Element Method

% Train Parameters
% mc = mass of car body
% mm = mass of magnet
% k = stiffness of suspension
% c = damping of suspension
% i0 = nominal current
% h0 = nominal air gap
...
function [t,yv,vv,av] = solve_model3_fem(mc,mm,k,c,i0,h0,kh,kv,ka,V, ...
...
Pv0 = [mc*9.81;mm*9.81-F0+(Ci*kh-Ch)*(ub(1,1)+h0)];
av(:,1) = Mv\(Pv0-Cv*vv(:,1)-Kv*yv(:,1));
F = mm*9.81-mm*av(2,1)-c*(vv(2,1)-vv(1,1))-k*(yv(2,1)-yv(1,1));
Pb0 = zeros(2*n+s-1,1);
Pb0(1) = F;
...
for i = 2:length(t)
    ...
    while err > 1e-8
        ...
        element_cur = floor((i-1)/num_dt)+1;
        x = mod(i-1,num_dt)*Le/num_dt;
        if element_cur > s*e
            element_cur = s*e;
            x = Le;
        end
        ...
        Pv = [mc*9.81;mm*9.81-F0+(Ci*kh-Ch)*(u+h0)];
        ...
        F = mm*9.81-mm*av_cur(2)-c*(vv_cur(2)-vv_cur(1))-k*(yv_cur(2)-yv_cur(1));
        Pb = zeros(2*n+s-1,1);
        Pb([dof1,dof2,dof3,dof4]) = F*[N1;N2;N3;N4];
