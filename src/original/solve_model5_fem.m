% Maglev Model 
% 6-DOF Train on Flexible Guideway
...
function [t,yv,vv,av] = solve_model5_fem(mc,Jc,mb,Jb,l,mm,kp,cp,ks,cs,i0,h0,kh,kv,ka,V, ...
    L,mg,EI,kb,kc,rayleighM,rayleighK,s,e, ...
    yv0,vv0,ub0,vb0,num_dt)

% Train
F0 = (mc+mb+2*mm)*9.81/2;
K0 = F0/(i0/h0)^2;
Ci = 2*K0*i0/h0^2;
Ch = 2*K0*i0^2/h0^3;
...
for j = 1:2
    ...
    Pv0(j+4) = mm*9.81-F0+(Ci*kh-Ch)*(u+h0);
end
...
for j = 1:2
    if train_pos(j) >= 0 && train_pos(j) <= s*L
        ...
        dof3 = element_info(element_cur,4);
        dof4 = element_info(element_cur,5);
        F = mm*9.81-mm*av(j+4,1)-cp*(vv(j+4,1)-(vv(3,1)+Lc(j)*vv(4,1)))-kp*(yv(j+4,1)-(yv(3,1)+Lc(j)*yv(4,1)));
...
for i = 2:length(t)
    ...
    while err > 1e-8
        ...
        train_pos = (i-1)*L/e/num_dt*ones(1,2)+l*[-1,1];
        ...
        for j = 1:2
            ...
            Pv(j+4) = mm*9.81-F0+(Ci*kh-Ch)*(u+h0);
        end
        ...
        for j = 1:2
            if train_pos(j) >= 0 && train_pos(j) <= s*L
                ...
                dof3 = element_info(element_cur,4);
                dof4 = element_info(element_cur,5);
                F = mm*9.81-mm*av_cur(j+4)-cp*(vv_cur(j+4)-(vv_cur(3)+Lc(j)*vv_cur(4)))-kp*(yv_cur(j+4)-(yv_cur(3)+Lc(j)*yv_cur(4)));
